...
def block(config, images, block_type, win, fixation, clock, screen_res, answers, answers_buttons, mouse, feedback,
    ...
    for trial in images:
        ...
        if config["answers_type"] == "keyboard" and len(answer) < config["n_answers"]:
            win.flip()
            while clock.getTime() < config["answer_time"]:
                show_clock(clock_image, clock, config)
                show_timer(timer, clock, config)
                ans = event.getKeys(keyList=config["reaction_keys"])
                if ans:
        ...
        # draw trial for answer_type == mouse
            ...
            win.flip()
            while clock.getTime() < config["answer_time"] and len(answer) < config["n_answers"]:
                show_clock(clock_image, clock, config)
                show_timer(timer, clock, config)
                for k, ans_button in answers_buttons.items():
                    if mouse.isPressedIn(ans_button) and str(k) not in answer:
                        reaction_time = clock.getTime()
                        answer.append(str(k))
                        if len(answer) == config["n_answers"]:
                            break
        ...
        if config["n_answers"] > 1:
            correct_answer = answers.loc[(answers['item_type'] == block_type) &
                                         (answers['item_id'] == trial["image_ID"])]['answer'].iloc[0]
            correct_answer = correct_answer.strip('][').split(',')
            correct_answer.sort()
            answer.sort()
        else:
